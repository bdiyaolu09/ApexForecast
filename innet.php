...
$sql = "SELECT net_profit_margin, year FROM user_info WHERE email = '$email' ORDER BY year ASC";
...
while ($row = $result->fetch_assoc()) {
    if ($prev_opm !== null) {
        ...
        $opm[] = ($row['net_profit_margin'] - $prev_opm) / $prev_opm_divisor;
    }
    $prev_opm = $row['net_profit_margin'];
}

// Get the turnover rate data for the industry from the database
$sql = "SELECT net_profit_margin FROM industry1 ORDER BY year ASC";
$result = $conn->query($sql);

// Calculate the industry growth rate between each year's turnover rate and the previous year's turnover rate
$industry_growth_rates = [];
$prev_turnover_rate = null;
while ($row = $result->fetch_assoc()) {
    if ($prev_turnover_rate !== null) {
        $prev_turnover_rate_divisor = ($prev_turnover_rate == 0 ? 1 : $prev_turnover_rate); // check if previous turnover rate is zero
        $industry_growth_rates[] = ($row['net_profit_margin'] - $prev_turnover_rate) / $prev_turnover_rate_divisor;
    }
    $prev_turnover_rate = $row['net_profit_margin'];
}

// Predict the next five years' operating profit margin based on the year-to-year growth rate and the industry growth rate
$predictions = [];
$prev_growth_rate = end($opm);
$prev_industry_growth_rate = end($industry_growth_rates);
for ($i = 1; $i <= 5; $i++) {
    // Calculate the predicted operating profit margin based on the previous year's operating profit margin, the previous year's growth rate, and the previous year's industry growth rate
    $predicted_opm = $prev_opm * (1 + $prev_growth_rate + $prev_industry_growth_rate);
...
<body>
    <header class = "header">
        <h1>Industry Trends: Predicted Net Profit Margin Dashboard</h1>
    </header>
           ...
           <script>
            // Retrieve the JSON data from PHP
             ...
             var chartData = {
                 labels: ['1', '2', '3', '4', '5'],
                 datasets: [{
                  label: 'Predicted Net Profit Margin %',
                  data: predictedData,
                  borderColor: 'blue',
