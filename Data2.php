...
// storing row data
//check if data already exists for the given email and year
$select_query = "SELECT * FROM Data WHERE email='$email' AND year= 2 ";
$result = mysqli_query($connection, $select_query);

if(mysqli_num_rows($result) > 0) {
    echo 'me<br>';
  //update the existing data in the database
  $update_query = "UPDATE Data SET industry='$industry', net_sales =$net_sales, revenue=$revenue, cogs=$cogs, operating_profit=$operating_profit,total_cost=$total_cost,aiasp=$aiasp  WHERE email='$email' AND year= 2 ";
...
//redirect to year 2 page
header("Location: year2_data.php");
exit;
?>
