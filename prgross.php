...
$sql = "SELECT gross_profit_margin, year FROM user_info WHERE email = '$email' ORDER BY year ASC";
...
while ($row = $result->fetch_assoc()) {
    if ($prev_opm !== null) {
        ...
        $opm[] = ($row['gross_profit_margin'] - $prev_opm) / $prev_opm_divisor;
    }
    $prev_opm = $row['gross_profit_margin'];
}
...
//echo "<h3>Predicted Operating Profit Margin for the Next Five Years:</h3>";
//echo "<ul>";
...
.card {
    background: linear-gradient(45deg, gold, #960018);
    border-radius: 10px;
...
<body>
    <header class = "header">
        <h1>Trend Analysis: Predicted Gross Profit Margin Dashboard</h1>
    </header>
           ...
           <script>
            // Retrieve the JSON data from PHP
             ...
             var chartData = {
                 labels: ['1', '2', '3', '4', '5'],
                 datasets: [{
                  label: 'Predicted Gross Profit Margin %',
                  data: predictedData,
                  borderColor: '#960018',
