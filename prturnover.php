...
$sql = "SELECT turnover_rate, year FROM user_info WHERE email = '$email' ORDER BY year ASC";
...
while ($row = $result->fetch_assoc()) {
    if ($prev_opm !== null) {
        ...
        $opm[] = ($row['turnover_rate'] - $prev_opm) / $prev_opm_divisor;
    }
    $prev_opm = $row['turnover_rate'];
}
...
.card {
    background: linear-gradient(45deg, gold, #000080);
    border-radius: 10px;
...
<body>
    <header class = "header">
        <h1>Trend Analysis: Predicted Turnover Rates Dashboard</h1>
    </header>
